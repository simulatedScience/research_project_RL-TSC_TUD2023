...
def load_files_to_csv(file_paths):
    ...
    data = []
    header = ['exp_identifier', 'id', 'fc', 'tpr', 'fpr', 'travel time', 'throughput', 'queue', 'delay', 'rewards']
    data.append(header)
        ...
        with open(file_path, 'r') as file:
            while True:
                ...
                if ':' in lines[0] and ':' in lines[1] and ',' in lines[2]:
                    ...
                    id_fc_tpr_fpr = lines[1].split(':')[1].split('_')
                    id = id_fc_tpr_fpr[0].split('=')[1]
                    fc = id_fc_tpr_fpr[1].split('=')[1]
                    tpr = id_fc_tpr_fpr[2].split('=')[1]
                    fpr = id_fc_tpr_fpr[3].split('=')[1]
                    ...
                    data.append([exp_identifier, id, fc, tpr, fpr, travel_time, throughput, queue, delay, rewards])
    return data
